package com.sulake.core.window.utils
{
   import com.sulake.core.window.enum.WindowParam;
   import flash.utils.Dictionary;
   
   public class ParamCodeTable extends WindowParam
   {
       
      
      public function ParamCodeTable()
      {
         super();
      }
      
      public static function fillTables(param1:Dictionary, param2:Dictionary = null) : void
      {
         var _loc3_:* = null;
         param1["null"] = const_106;
         param1["bound_to_parent_rect"] = const_89;
         param1["child_window"] = const_1010;
         param1["embedded_controller"] = const_321;
         param1["resize_to_accommodate_children"] = const_64;
         param1["input_event_processor"] = const_50;
         param1["internal_event_handling"] = const_508;
         param1["mouse_dragging_target"] = const_227;
         param1["mouse_dragging_trigger"] = const_287;
         param1["mouse_scaling_target"] = const_257;
         param1["mouse_scaling_trigger"] = const_440;
         param1["horizontal_mouse_scaling_trigger"] = const_195;
         param1["vertical_mouse_scaling_trigger"] = const_206;
         param1["observe_parent_input_events"] = const_810;
         param1["optimize_region_to_layout_size"] = const_391;
         param1["parent_window"] = const_932;
         param1["relative_horizontal_scale_center"] = const_167;
         param1["relative_horizontal_scale_fixed"] = const_130;
         param1["relative_horizontal_scale_move"] = const_337;
         param1["relative_horizontal_scale_strech"] = const_253;
         param1["relative_scale_center"] = const_970;
         param1["relative_scale_fixed"] = const_684;
         param1["relative_scale_move"] = const_801;
         param1["relative_scale_strech"] = const_841;
         param1["relative_vertical_scale_center"] = const_166;
         param1["relative_vertical_scale_fixed"] = const_115;
         param1["relative_vertical_scale_move"] = const_311;
         param1["relative_vertical_scale_strech"] = const_266;
         param1["on_resize_align_left"] = const_624;
         param1["on_resize_align_right"] = const_383;
         param1["on_resize_align_center"] = const_428;
         param1["on_resize_align_top"] = const_668;
         param1["on_resize_align_bottom"] = const_357;
         param1["on_resize_align_middle"] = const_352;
         param1["on_accommodate_align_left"] = const_965;
         param1["on_accommodate_align_right"] = const_443;
         param1["on_accommodate_align_center"] = const_721;
         param1["on_accommodate_align_top"] = const_865;
         param1["on_accommodate_align_bottom"] = WINDOW_PARAM_ON_ACCOMMODATE_ALIGN_BOTTOM;
         param1["on_accommodate_align_middle"] = const_645;
         param1["route_input_events_to_parent"] = const_351;
         param1["use_parent_graphic_context"] = const_32;
         param1["draggable_with_mouse"] = const_802;
         param1["scalable_with_mouse"] = const_898;
         param1["reflect_horizontal_resize_to_parent"] = const_456;
         param1["reflect_vertical_resize_to_parent"] = WINDOW_PARAM_REFLECT_VERTICAL_RESIZE_TO_PARENT;
         param1["reflect_resize_to_parent"] = const_269;
         if(param2 != null)
         {
            for(_loc3_ in param1)
            {
               if(param2[param1[_loc3_]] != null)
               {
                  if(param1[_loc3_] != 0)
                  {
                     Logger.log("Conflictiong flags in window params: \"" + _loc3_ + "\"!");
                  }
               }
               param2[param1[_loc3_]] = _loc3_;
            }
         }
      }
   }
}
