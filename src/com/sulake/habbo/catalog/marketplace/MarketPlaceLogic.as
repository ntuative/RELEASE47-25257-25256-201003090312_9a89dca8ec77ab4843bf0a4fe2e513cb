package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private var var_1704:MarketplaceItemStats;
      ...
      public function buyOffer(param1:int) : void
      {
         ...
         showConfirmation(const_1485,_loc2_);
      }
      ...
      {
         if(param1.furniCategoryId != var_1703 || param1.furniTypeId != var_1705)
         {
            return;
         }
         var_1704 = param1;
      ...
      public function onCancelResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            item = var_185.remove(parser.method_2);
            if(item != null)
            {
               item.dispose();
            }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         else if(parser.result == 3)
         {
            ...
            if(updateItem)
            {
               updateItem.method_2 = parser.method_2;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               var_148.add(parser.method_2,updateItem);
            }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_185.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_185.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_397)
            {
               var_185.remove(_loc2_);
               _loc3_.dispose();
            }
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         var_1710 = _loc3_.creditsWaiting;
         var_185 = new Map();
         for each(_loc4_ in _loc3_.offers)
         {
            _loc6_ = new MarketPlaceOfferData(_loc4_.method_2,_loc4_.furniId,_loc4_.furniType,_loc4_.price,_loc4_.status,_loc4_.averagePrice);
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            var_185.add(_loc4_.method_2,_loc6_);
         }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            _loc6_ = new MarketPlaceOfferData(_loc4_.method_2,_loc4_.furniId,_loc4_.furniType,_loc4_.price,_loc4_.status,_loc4_.averagePrice,_loc4_.offerCount);
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            var_148.add(_loc4_.method_2,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         requestOffers(var_1707,var_1706,var_1708,var_1709);
      }
